...
#====================================================================================================
#
#	Download by https://github.com/SavinaRoja/PyUserInput the module "PyUserInput"
#	This package contains more files. To install this module into Python path 
#   execute the command "python setup.py install". The setup file is located into
#   default directory "PyUserInput-master". The complete path depends where you are
#   unzipped the package
#
#	Download by https://github.com/tipam/pi3d the module "pi3D"
#	This package contains more files. To install this module into Python path 
#   execute the command "python setup.py install". The setup file is located into
#   default directory "pi3d-master". The complete path depends where you are
#   unzipped the package
#
#	If necessary install the "python-xlib" library. Use this command to install
#   library: "apt-get install python-xlib"
#
#	Install the image viewer "Eye of Gnome" with the command "apt-get install eog"
#	The online help is located at "https://help.gnome.org/users/eog/stable/"
#	For keyboard shortcut is located at "https://help.gnome.org/users/eog/stable/shortcuts.html.en"
#
#	NOTE: If you are not administrator user it is necessary to add the suffix "sudo"
...
def main():
	...
	while True:
		...
		if (ActionInProgress == True):
			...
			if (MGC3130._GestOutput.Gesture32Bit.GestSouthNorth == 1):
				MGC3130._GestOutput.Gesture32Bit.GestSouthNorth = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
				KeyPressed.tap_key(KeyPressed.up_key)
		
			if (MGC3130._GestOutput.Gesture32Bit.GestNorthSouth == 1):
				MGC3130._GestOutput.Gesture32Bit.GestNorthSouth = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
				KeyPressed.tap_key(KeyPressed.down_key)

			if (MGC3130._GestOutput.Gesture32Bit.TouchCentre == 1):
				MGC3130._GestOutput.Gesture32Bit.TouchCentre = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
				KeyPressed.tap_key(KeyPressed.function_keys[5])
								
			if (MGC3130._GestOutput.Gesture32Bit.TouchSouth == 1):
				MGC3130._GestOutput.Gesture32Bit.TouchSouth = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
				if (ZoomStep > 0):
			...
			if (MGC3130._GestOutput.Gesture32Bit.TouchNorth == 1):
				MGC3130._GestOutput.Gesture32Bit.TouchNorth = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
			...
			if (MGC3130._GestOutput.Gesture32Bit.GestClockWise == 1):
				MGC3130._GestOutput.Gesture32Bit.GestClockWise = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
			...
			if (MGC3130._GestOutput.Gesture32Bit.GestCounterClockWise == 1):
				MGC3130._GestOutput.Gesture32Bit.GestCounterClockWise = 0
				GPIO.output(LED_DIAGNOSTIC, GPIO.LOW)
				BaseTimeTimer = threading.Timer(1, SlowBaseTime)
				BaseTimeTimer.start()
