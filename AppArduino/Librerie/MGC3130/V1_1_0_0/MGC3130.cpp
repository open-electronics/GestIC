...
void MGC3130::DecodeGesture(void) {
	...
	const char GestureWaveX[] 			 	PROGMEM = "Gesture Wave X";	
	const char GestureWaveY[] 				PROGMEM = "Gesture Wave Y";
	const char GestureHold[] 			 	PROGMEM = "Gesture Hold";	
	const char GesturePresence[]			PROGMEM = "Gesture Presence";

	const char GestureDoubleWestToEast[]   	PROGMEM = "Gesture Double West to East";
	const char GestureDoubleEastToWest[]   	PROGMEM = "Gesture Double East to West";
	const char GestureDoubleNorthToSouth[] 	PROGMEM = "Gesture Double North to South";
	const char GestureDoubleSouthToNorth[] 	PROGMEM = "Gesture Double South to North";
	...
	void MGC3130::PrintMGC3130Gesture(void) {
		...
		if (GestureOutput.Bit.GestWaveX > 0) {
			ReadStringFLASH((uint8_t *)GestureWaveX, strlen(GestureWaveX), TRUE);
		}		
		if (GestureOutput.Bit.GestWaveY > 0) {
			ReadStringFLASH((uint8_t *)GestureWaveY, strlen(GestureWaveY), TRUE);
		}
		if (GestureOutput.Bit.GestHold > 0) {
			ReadStringFLASH((uint8_t *)GestureHold, strlen(GestureHold), TRUE);
		}		
		if (GestureOutput.Bit.GestPresence > 0) {
			ReadStringFLASH((uint8_t *)GesturePresence, strlen(GesturePresence), TRUE);
		}	
		//----------------------------------------
		
		//----------------------------------------
		if (GestureOutput.Bit.DoubleGestWestEast > 0) {
			ReadStringFLASH((uint8_t *)GestureDoubleWestToEast, strlen(GestureDoubleWestToEast), TRUE);
		}		
		if (GestureOutput.Bit.DoubleGestEastWest > 0) {
			ReadStringFLASH((uint8_t *)GestureDoubleEastToWest, strlen(GestureDoubleEastToWest), TRUE);
		}	
		if (GestureOutput.Bit.DoubleSouthNorth > 0) {
			ReadStringFLASH((uint8_t *)GestureDoubleSouthToNorth, strlen(GestureDoubleSouthToNorth), TRUE);
		}	
		if (GestureOutput.Bit.DoubleGestNorthSouth > 0) {
			ReadStringFLASH((uint8_t *)GestureDoubleNorthToSouth, strlen(GestureDoubleNorthToSouth), TRUE);
		}	
		//----------------------------------------
	}
#endif
